...
hand = p.loadURDF("urdf/gx11.urdf", useFixedBase=True, basePosition=[0, 0, 0])
...
tx = p.addUserDebugParameter(f"TX", 0, 0.3, 0.03)
...
while True:
    ...
    # 计算逆运动学
    joint_positions = p.calculateInverseKinematics(hand, 3, xyz, maxNumIterations=300, residualThreshold=1e-4)
    ...
    xyz_now5 = p.getLinkState(hand, 3, computeForwardKinematics=1)[4]
    # xyz_now1 = p.getLinkState(hand, 3, computeForwardKinematics=1)[0]
